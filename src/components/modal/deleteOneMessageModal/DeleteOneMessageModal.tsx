...
export default function DeleteOneMessageModal({
  onToggle,
  messageId,
}: {
  onToggle: () => void;
  ...
  async function handleDeleteMessage() {
    const data = await deleteMessage(messageId, userMessage?.id as string);
      ...
      >
        ...
        <div className="flex flex-col items-center text-center gap-6 mt-10">
          ...
          <span className="font-dmSans w-[180px] text-gray-800 font-bold tracking-[-0.5px]">
            Tem certeza que excluir essa mensagem?
          </span>
          <p className="font-dmSans tracking-[-0.5px] text-gray-800">
            Ao excluir essa mensagem não sera possivel voltar atras, ela sera
